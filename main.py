...
def play(player,tabuleiro, submarinos, cruzadores, porta_avioes):   
	while True:
		...
		if hitNavio(coordenada, submarinos, tabuleiro, "S", "submarino") or hitNavio(coordenada, cruzadores, tabuleiro, "C", "cruzador") or hitNavio(coordenada, porta_avioes, tabuleiro, "P", "porta aviao"):
			print("-" * 10)
			print("FOGO")
			print("-" * 10)	
...
def hitASubmarino(coordenada, submarinos, tabuleiro):
	...
	return False
'''
def hitNavio(coordenada, navio, tabuleiro, s, mensagem):
	for i in navio:
		if coordenada in i:
			linha, coluna = coordenada[0], coordenada[1] 
			tabuleiro[linha][coluna] = s
